...
class HermesAgentLauncherTests(unittest.TestCase):
    ...
    def test_credential_erase_survives_exact_parent_replacement_after_quarantine(self) -> None:
        path = self.runs / "hostile.credential"
        path.write_bytes(b"synthetic-secret")
        path.chmod(0o600)
        original_fd = os.open(path, os.O_RDONLY | getattr(os, "O_CLOEXEC", 0))
        moved_parent = self.root / "runs-hostile-original"
        replacement_bytes = b"replacement-credential"
        original_open = launcher._open_private_entry
        replaced = False

        def hostile_open(parent_fd, name, *, writable, code):
            nonlocal replaced
            if name.startswith(".cleanup-") and not replaced:
                replaced = True
                self.runs.rename(moved_parent)
                self.runs.mkdir(mode=launcher.live_run_marker.RUNS_DIR_MODE)
                self.runs.chmod(launcher.live_run_marker.RUNS_DIR_MODE)
                replacement = self.runs / path.name
                replacement.write_bytes(replacement_bytes)
                replacement.chmod(0o600)
                raise launcher.LauncherError("hostile_quarantine_open_failed")
            return original_open(parent_fd, name, writable=writable, code=code)

        try:
            with mock.patch.object(launcher, "_open_private_entry", side_effect=hostile_open):
                with self.assertRaises(launcher.LauncherError) as raised:
            ...
            os.lseek(original_fd, 0, os.SEEK_SET)
            self.assertEqual(os.read(original_fd, launcher.live_run_marker.MAX_CREDENTIAL_BYTES), b"")
        finally:
            os.close(original_fd)
        self.assertEqual(raised.exception.code, "hostile_quarantine_open_failed")
        self.assertTrue(replaced)
        self.assertEqual((self.runs / path.name).read_bytes(), replacement_bytes)
        self.assertEqual((moved_parent / ".cleanup-00").read_bytes(), b"")
