...
enum MockWorkloadExecutor {
    ...
    private static func streamingBatchReduction(operation: WorkloadOperationSpec, seed: UInt64) -> UInt64 {
        var generator = SeededFixtureGenerator(seed: seed)
    ...
    private static func transcriptDiffAndScroll(operation: WorkloadOperationSpec, seed: UInt64) -> UInt64 {
        var oldGenerator = SeededFixtureGenerator(seed: seed)
        var newGenerator = SeededFixtureGenerator(seed: seed &+ 0x517cc1b727220a95)
        ...
        for index in 0..<operation.itemCount {
            ...
            let newText = index.isMultiple(of: 7)
