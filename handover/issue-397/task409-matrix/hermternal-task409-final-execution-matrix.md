...
- Driver shell SHA-256: `1599b4a0155c5963f5272ca18377cbfbee170ce6e92372ec643dfac3b256ab26`
...
- Fresh extraction boundary: extract `/execution_driver/shell` from this exact Markdown/JSON pair, require shell SHA-256 `1599b4a0155c5963f5272ca18377cbfbee170ce6e92372ec643dfac3b256ab26`, require normalized JSON identity `94774470b8cd9a6a7ec8e6b480883762d432b845355fa5520b398d9b60fdb6d8`, then invoke only the freshly extracted body. Standalone `/private/tmp/hermternal-task409-approved-driver.sh` is stale and non-authoritative; never reuse it.
...
- Matrix identity: exact source bytes are validated with normalized SHA-256 `94774470b8cd9a6a7ec8e6b480883762d432b845355fa5520b398d9b60fdb6d8`; after root creation they are read once through `O_NOFOLLOW`, written with `O_CREAT|O_EXCL` at mode `0600` to `$REPLAY_ROOT/matrix.snapshot.json`, and every mutation boundary verifies device/inode/size/realpath.
- Byte-exact Markdown extraction: raw bytes, canonical section heading, first exact ` ```bash\n ` opening, first exact ` ``` ` closing, and no trimming or newline normalization; body and JSON shell are `184832` bytes, SHA-256 `1599b4a0155c5963f5272ca18377cbfbee170ce6e92372ec643dfac3b256ab26`, terminal byte `0a`.
...
readonly MATRIX_EXPECTED_BINDING_SHA256='94774470b8cd9a6a7ec8e6b480883762d432b845355fa5520b398d9b60fdb6d8'
MATRIX_BOUND=0
...
raw = subprocess.check_output([
    '/usr/bin/git', '--no-replace-objects', '--no-lazy-fetch', '--no-optional-locks', '-C', repo, '-c', 'core.hooksPath=/dev/null', '-c', 'protocol.allow=never', '-c', 'protocol.allow=never',
...
git_dir = subprocess.check_output(['/usr/bin/git', '--no-replace-objects', '--no-lazy-fetch', '--no-optional-locks', '-C', repo, '-c', 'core.hooksPath=/dev/null', '-c', 'protocol.allow=never', '-c', 'protocol.allow=never', 'rev-parse', '--absolute-git-dir'], text=True).strip()
...
def run(*args, check=True):
    p = subprocess.run(['/usr/bin/git', '--no-replace-objects', '--no-lazy-fetch', '--no-optional-locks', '-C', repo, '-c', 'core.hooksPath=/dev/null', '-c', 'protocol.allow=never', '-c', 'protocol.allow=never', *args], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
for path in sys.argv[2:]:
    p = subprocess.run(['/usr/bin/git', '--no-replace-objects', '--no-lazy-fetch', '--no-optional-locks', '-C', repo, '-c', 'core.hooksPath=/dev/null', '-c', 'protocol.allow=never', '-c', 'protocol.allow=never', 'ls-files', '--stage', '--', path], stdout=subprocess.PIPE, check=True)
    raw = p.stdout.decode()
    if not raw:
    ...
    if not os.path.isfile(full) or os.path.islink(full):
        raise SystemExit(f'worktree type mismatch for {path}')
    actual = subprocess.check_output(['/usr/bin/git', '--no-replace-objects', '--no-lazy-fetch', '--no-optional-locks', '-C', repo, '-c', 'core.hooksPath=/dev/null', '-c', 'protocol.allow=never', '-c', 'protocol.allow=never', 'hash-object', '--', path], text=True).strip()
    if actual != blob:
        raise SystemExit(f'worktree blob mismatch for {path}')
    actual_mode = '100755' if stat.S_IXUSR & os.stat(full).st_mode else '100644'
    if actual_mode != mode:
        raise SystemExit(f'worktree mode mismatch for {path}')
    subprocess.run(['/usr/bin/git', '--no-replace-objects', '--no-lazy-fetch', '--no-optional-locks', '-C', repo, '-c', 'core.hooksPath=/dev/null', '-c', 'protocol.allow=never', '-c', 'protocol.allow=never', 'diff-files', '--quiet', '--', path], check=True)
...
git = '/usr/bin/git'
def show(spec):
    return subprocess.check_output([git, '--no-replace-objects', '--no-lazy-fetch', '--no-optional-locks', '-C', str(repo), '-c', 'core.hooksPath=/dev/null', '-c', 'protocol.allow=never', '-c', 'protocol.allow=never', 'show', spec])
...
def run(*args, check=True, input_bytes=None):
    p = subprocess.run([git, '--no-replace-objects', '--no-lazy-fetch', '--no-optional-locks', '-C', repo, '-c', 'core.hooksPath=/dev/null', '-c', 'protocol.allow=never', '-c', 'protocol.allow=never', *args], input=input_bytes, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if check and p.returncode:
...
| JSON companion | `/private/private/private/tmp/hermternal-task409-final-execution-matrix.json` |
| Driver schema | `hermternal.task409.execution-driver.v1` |
| Driver shell SHA-256 | `1599b4a0155c5963f5272ca18377cbfbee170ce6e92372ec643dfac3b256ab26` |
...
| Swift source SHA | `221620c04bb051f2597c52bdeb16ccc55c5b2e9c` |
| Shell body bytes | `184832` |
| Shell body lines | `3880` |
