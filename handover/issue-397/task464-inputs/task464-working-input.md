...
| Authority | `/tmp/task409-old-runbook.txt` |
...
- Driver shell SHA-256: `1168e369d9b1f2116ae871e890ff6ab3b0489ac0c6c0f0925279083c7e2240dd`
...
- Source repository: `/Users/agents/Developer/hermternal` (read-only; source cleanliness is not required)
- Clean-primary root: `/private/tmp/hermternal-task409-final-clean-primary.$$` with repository `/private/tmp/hermternal-task409-final-clean-primary.$$/repository`
- Replay root: `/private/tmp/hermternal-task409-final-replay.$$`
- Fresh extraction boundary: extract `/execution_driver/shell` from this exact Markdown/JSON pair, require shell SHA-256 `1168e369d9b1f2116ae871e890ff6ab3b0489ac0c6c0f0925279083c7e2240dd`, require normalized JSON identity `fff40200b6b29ef831b0cb69a8a36fc36a6a9e01dd13d06c9047a82a626da812`, then invoke only the freshly extracted body. Standalone `/private/tmp/hermternal-task409-approved-driver.sh` is stale and non-authoritative; never reuse it.
...
- Matrix identity: exact source bytes are validated with normalized SHA-256 `fff40200b6b29ef831b0cb69a8a36fc36a6a9e01dd13d06c9047a82a626da812`; after root creation they are read once through `O_NOFOLLOW`, written with `O_CREAT|O_EXCL` at mode `0600` to `$REPLAY_ROOT/matrix.snapshot.json`, and every mutation boundary verifies device/inode/size/realpath.
- Byte-exact Markdown extraction: raw bytes, canonical section heading, first exact ` ```bash\n ` opening, first exact ` ``` ` closing, and no trimming or newline normalization; body and JSON shell are `106051` bytes, SHA-256 `1168e369d9b1f2116ae871e890ff6ab3b0489ac0c6c0f0925279083c7e2240dd`, terminal byte `0a`.
...
| 3 | `create-dedicated-clean-primary-root` | `clean-primary-root` | fixed absolute /private/tmp root is atomically exclusive, mode 0700, canonical, non-symlink, and outside SOURCE/replay/worktrees |
...
| 5 | `create-dedicated-replay-root` | `replay-root` | absolute /private/tmp root outside SOURCE, clean-primary, and every registered/nested worktree |
...
readonly MARKDOWN=/private/tmp/hermternal-task409-final-execution-matrix.md
readonly MATRIX_SOURCE=/private/tmp/hermternal-task409-final-execution-matrix.json
MATRIX="$MATRIX_SOURCE"
readonly MATRIX_EXPECTED_BINDING_SHA256='fff40200b6b29ef831b0cb69a8a36fc36a6a9e01dd13d06c9047a82a626da812'
MATRIX_BOUND=0
  ...
  while IFS='=' read -r name ignored; do
    case "$name" in
      GIT_CONFIG_PARAMETERS|GIT_CONFIG_COUNT|GIT_CONFIG_KEY_*|GIT_CONFIG_VALUE_*|PYTHONPATH|PYTHONHOME|PYTHONSTARTUP|PYTHONUSERBASE|PYTHONINSPECT|PYTHONWARNINGS|BASH_ENV|ENV|CDPATH|NODE_OPTIONS|RUBYOPT|PERL5OPT|DYLD_*|LD_*)
        fail "forbidden inherited environment variable: $name" ;;
      PATH|HOME|LANG|LC_ALL|GIT_CONFIG_NOSYSTEM|GIT_CONFIG_GLOBAL|GIT_CONFIG_SYSTEM|GIT_TERMINAL_PROMPT|GIT_OPTIONAL_LOCKS|GIT_NO_REPLACE_OBJECTS|PWD|SHLVL|_) ;;
      *) fail "environment variable is not allowlisted: $name" ;;
    esac
  done < <("$ENV")
}
assert_environment
...
create_clean_primary_root() {
  test "$CLEAN_PRIMARY_ROOT_BOUND" -eq 0 || fail "clean-primary root already bound"
  local identity
  identity="$($PYTHON - "$CLEAN_PRIMARY_ROOT" <<'PY'
import os
import stat
import sys
root = sys.argv[1]
...
if stat.S_ISLNK(parent_st.st_mode) or not stat.S_ISDIR(parent_st.st_mode) or os.path.realpath(parent) != parent:
    raise SystemExit('clean-primary parent is not canonical')
if os.path.lexists(root):
    raise SystemExit('clean-primary root already exists')
try:
    os.mkdir(root, 0o700)
except FileExistsError:
    raise SystemExit('clean-primary root appeared during exclusive creation')
st = os.lstat(root)
if stat.S_ISLNK(st.st_mode) or not stat.S_ISDIR(st.st_mode) or stat.S_IMODE(st.st_mode) != 0o700 or os.path.realpath(root) != root:
    raise SystemExit('clean-primary root identity invalid after creation')
print(f'{st.st_dev}\t{st.st_ino}\t{st.st_uid}\t{stat.S_IMODE(st.st_mode):04o}\t{root}')
PY
)"
  IFS=$'\t' read -r CLEAN_PRIMARY_ROOT_DEVICE CLEAN_PRIMARY_ROOT_INODE CLEAN_PRIMARY_ROOT_UID CLEAN_PRIMARY_ROOT_MODE CLEAN_PRIMARY_ROOT_REALPATH <<< "$identity"
  test "$CLEAN_PRIMARY_ROOT_REALPATH" = "$CLEAN_PRIMARY_ROOT" || fail "clean-primary root canonical binding failed"
...
assert_clean_primary_state() {
  ...
  whole_worktree_status="$(git_primary status --porcelain=v1 --untracked-files=all --ignored=all)"
...
bootstrap_clean_primary() {
  ...
  "$GIT" -c core.hooksPath=/dev/null -c protocol.allow=never clone --local --no-hardlinks --no-checkout --no-tags "$SOURCE" "$CLEAN_PRIMARY" >/dev/null
  bind_clean_primary_identity
  before_clean_primary_mutation
...
create_replay_root() {
  test "$REPLAY_ROOT_BOUND" -eq 0 || fail "replay root already bound"
  local identity
  identity="$($PYTHON - "$REPLAY_ROOT" <<'PY'
import os
import stat
import sys
root = sys.argv[1]
...
parent_st = os.lstat(parent)
if stat.S_ISLNK(parent_st.st_mode) or not stat.S_ISDIR(parent_st.st_mode) or os.path.realpath(parent) != parent:
    raise SystemExit('replay parent is not canonical')
if os.path.lexists(root):
    raise SystemExit('replay root already exists')
try:
    os.mkdir(root, 0o700)
except FileExistsError:
    raise SystemExit('replay root appeared during exclusive creation')
st = os.lstat(root)
if stat.S_ISLNK(st.st_mode) or not stat.S_ISDIR(st.st_mode) or stat.S_IMODE(st.st_mode) != 0o700 or os.path.realpath(root) != root:
    raise SystemExit('replay root identity invalid after creation')
print(f'{st.st_dev}\t{st.st_ino}\t{st.st_uid}\t{stat.S_IMODE(st.st_mode):04o}\t{root}')
PY
)"
  IFS=$'\t' read -r REPLAY_ROOT_DEVICE REPLAY_ROOT_INODE REPLAY_ROOT_UID REPLAY_ROOT_MODE REPLAY_ROOT_REALPATH <<< "$identity"
  test "$REPLAY_ROOT_REALPATH" = "$REPLAY_ROOT" || fail "replay root identity was not captured"
  REPLAY_ROOT_BOUND=1
  assert_root_shape
}
...
read_fd = os.open(snapshot, os.O_RDONLY | os.O_NOFOLLOW)
try:
    read_chunks = []
    while True:
...
assert_bash32_array_smoke() {
  ...
  local saved_dirty=() item count
  saved_dirty=(${DIRTY_PATHS[@]+"${DIRTY_PATHS[@]}"})
  DIRTY_PATHS=()
  count=0
  for item in ${DIRTY_PATHS[@]+"${DIRTY_PATHS[@]}"}; do
    count=$((count + 1))
  done
  ...
  DIRTY_PATHS=(${saved_dirty[@]+"${saved_dirty[@]}"})
...
assert_root_shape() {
  ...
  "$PYTHON" - "$SOURCE" "$CLEAN_PRIMARY_ROOT" "$CLEAN_PRIMARY" "$REPLAY_ROOT" "$CLEAN_PRIMARY_ROOT_BOUND" "$CLEAN_PRIMARY_ROOT_DEVICE" "$CLEAN_PRIMARY_ROOT_INODE" "$CLEAN_PRIMARY_ROOT_UID" "$CLEAN_PRIMARY_ROOT_MODE" "$CLEAN_PRIMARY_ROOT_REALPATH" "$CLEAN_PRIMARY_BOUND" "$REPLAY_ROOT_BOUND" "$REPLAY_ROOT_DEVICE" "$REPLAY_ROOT_INODE" "$REPLAY_ROOT_UID" "$REPLAY_ROOT_MODE" "$REPLAY_ROOT_REALPATH" <<'PY'
import os
import stat
import sys
(source, clean_root, clean_repo, replay_root, clean_bound, clean_dev, clean_ino, clean_uid, clean_mode, clean_real, repo_bound, replay_bound, replay_dev, replay_ino, replay_uid, replay_mode, replay_real) = sys.argv[1:]
def verify(path, expected, values, label, mode=None):
    st = os.lstat(path)
    if stat.S_ISLNK(st.st_mode) or not stat.S_ISDIR(st.st_mode) or os.path.realpath(path) != expected or expected != path:
        raise SystemExit(label + ' path changed')
    actual = [str(st.st_dev), str(st.st_ino), str(st.st_uid), format(stat.S_IMODE(st.st_mode), '04o')]
...
if clean_bound == '0':
    ...
    verify(clean_root, clean_real, [clean_dev, clean_ino, clean_uid, clean_mode], 'clean-primary root', 0o700)
    if repo_bound == '0':
        if os.path.lexists(clean_repo):
            raise SystemExit('unbound clean-primary repository exists')
    elif os.path.islink(clean_repo) or not os.path.isdir(clean_repo) or os.path.realpath(clean_repo) != clean_repo:
        raise SystemExit('clean-primary repository path changed')
if replay_bound == '0':
    if os.path.lexists(replay_root):
        raise SystemExit('unbound replay root exists')
else:
    verify(replay_root, replay_real, [replay_dev, replay_ino, replay_uid, replay_mode], 'replay root', 0o700)
clean_real = os.path.realpath(clean_root)
replay_real = os.path.realpath(replay_root)
if overlaps(source_real, clean_real) or overlaps(clean_real, source_real):
    raise SystemExit('SOURCE and clean-primary roots overlap')
if overlaps(source_real, replay_real) or overlaps(replay_real, source_real):
    raise SystemExit('SOURCE and replay roots overlap')
if overlaps(clean_real, replay_real) or overlaps(replay_real, clean_real):
    raise SystemExit('clean-primary and replay roots overlap')
PY
}
...
if (first.st_dev, first.st_ino, first.st_size) != (second.st_dev, second.st_ino, second.st_size):
    raise SystemExit('matrix snapshot descriptor changed while reading')
...
assert_worktree_separation() {
  "$PYTHON" - "$SOURCE" "$CLEAN_PRIMARY" "$REPLAY" "$CLEAN_PRIMARY_BOUND" "$REPLAY_ROOT_BOUND" <<'PY'
import os
import subprocess
import sys
source, clean, replay, clean_bound, replay_bound = sys.argv[1:]
repos = [('source', source)]
if clean_bound == '1':
    repos.append(('clean-primary', clean))
if replay_bound == '1' and os.path.isdir(replay) and not os.path.islink(replay):
    repos.append(('replay', replay))
roots = [(label, os.path.realpath(path)) for label, path in repos]
if len({path for _, path in roots}) != len(roots):
    raise SystemExit('SOURCE/clean-primary/replay roots are not distinct')
def overlaps(a, b):
    try:
        return a == b or os.path.commonpath([a, b]) in (a, b)
    except ValueError:
...
for owner, repo in repos:
    output = subprocess.check_output(['/usr/bin/git', '-C', repo, '-c', 'core.hooksPath=/dev/null', 'worktree', 'list', '--porcelain'], text=True)
    for line in output.splitlines():
        if not line.startswith('worktree '):
            continue
        worktree = os.path.realpath(line[9:])
        for label, root in roots:
            if owner == label and worktree == root:
                continue
            if overlaps(worktree, root):
                raise SystemExit('registered/nested worktree overlap: ' + worktree)
PY
}
...
raw = subprocess.check_output([
    '/usr/bin/git', '-C', repo, '-c', 'core.hooksPath=/dev/null',
...
git_dir = subprocess.check_output(['/usr/bin/git', '-C', repo, '-c', 'core.hooksPath=/dev/null', 'rev-parse', '--absolute-git-dir'], text=True).strip()
...
before_mutation() {
  ...
  if [ "$REPLAY_READY" -eq 1 ]; then
    assert_replay_state "$CURRENT_HEAD" "$CURRENT_TREE" ${DIRTY_PATHS[@]+"${DIRTY_PATHS[@]}"}
  else
    assert_bootstrap_state
  fi
}
...
add_dirty_path() {
  local candidate="$1"
  local existing
  for existing in ${DIRTY_PATHS[@]+"${DIRTY_PATHS[@]}"}; do
...
def run(*args, check=True):
    p = subprocess.run(['/usr/bin/git', '-C', repo, '-c', 'core.hooksPath=/dev/null', *args], stdout=subprocess.PIPE, stderr=subprocess.PIPE)
...
assert_commit_gate() {
  ...
  assert_exact_commit_paths "$head" ${paths[@]+"${paths[@]}"}
  raw="$(git_replay diff --binary --full-index --no-ext-diff --no-textconv "$parent" "$head" -- ${paths[@]+"${paths[@]}"})"
  test -n "$raw" || fail "post-commit binary patch is empty"
  git_replay diff --check "$parent" "$head" -- ${paths[@]+"${paths[@]}"}
...
commit_staged() {
  ...
  assert_exact_staged ${paths[@]+"${paths[@]}"}
  git_replay diff --cached --check -- ${paths[@]+"${paths[@]}"}
  staged_raw="$(git_replay diff --cached --binary --full-index --no-ext-diff --no-textconv -- ${paths[@]+"${paths[@]}"})"
  test -n "$staged_raw" || fail "empty staged binary patch"
  staged_patch="$(printf '%s' "$staged_raw" | "$GIT" patch-id --stable | "$CUT" -d' ' -f1)"
  ...
  assert_commit_gate "$parent" "$CURRENT_HEAD" "$expected_patch" ${paths[@]+"${paths[@]}"}
}
...
apply_range_lane() {
  ...
  while IFS=$'	' read -r source expected parent tree path_blob; do
    ...
    assert_exact_staged ${paths[@]+"${paths[@]}"}
    git_replay diff --cached --check -- ${paths[@]+"${paths[@]}"}
    test "$(binary_patch_id "$parent" "$source" ${paths[@]+"${paths[@]}"})" = "$expected" || fail "source patch-id mismatch for $source"
    assert_cached_patch "$expected" ${paths[@]+"${paths[@]}"}
    commit_staged "replay(task409): $lane:$source" "$expected" ${paths[@]+"${paths[@]}"}
  done < <(range_records "$lane")
}
...
for path in sys.argv[2:]:
    p = subprocess.run(['/usr/bin/git', '-C', repo, '-c', 'core.hooksPath=/dev/null', 'ls-files', '--stage', '--', path], stdout=subprocess.PIPE, check=True)
    raw = p.stdout.decode()
    if not raw:
    ...
    if not os.path.isfile(full) or os.path.islink(full):
        raise SystemExit(f'worktree type mismatch for {path}')
    actual = subprocess.check_output(['/usr/bin/git', '-C', repo, '-c', 'core.hooksPath=/dev/null', 'hash-object', '--', path], text=True).strip()
    if actual != blob:
        raise SystemExit(f'worktree blob mismatch for {path}')
    actual_mode = '100755' if stat.S_IXUSR & os.stat(full).st_mode else '100644'
    if actual_mode != mode:
        raise SystemExit(f'worktree mode mismatch for {path}')
    subprocess.run(['/usr/bin/git', '-C', repo, '-c', 'core.hooksPath=/dev/null', 'diff-files', '--quiet', '--', path], check=True)
...
apply_semantic_lane() {
  ...
  assert_exact_staged ${paths[@]+"${paths[@]}"}
  git_replay diff --cached --check -- ${paths[@]+"${paths[@]}"}
  commit_staged "replay(task409): $lane semantic projection" - ${paths[@]+"${paths[@]}"}
}
...
apply_matrix_stage() {
  ...
  for row in ${snapshots[@]+"${snapshots[@]}"}; do
    IFS=$'	' read -r path old_blob old_mode new_blob new_mode <<< "$row"
  ...
  assert_exact_staged ${paths[@]+"${paths[@]}"}
  git_replay diff --cached --check -- ${paths[@]+"${paths[@]}"}
  test "$(binary_patch_id "$source_left" "$target" ${paths[@]+"${paths[@]}"})" = "$expected_patch" || fail "approved mechanical source patch-id mismatch for $stage"
  test "$(raw_patch_sha256 "$source_left" "$target" ${paths[@]+"${paths[@]}"})" = "$expected_raw" || fail "approved mechanical source raw SHA mismatch for $stage"
  assert_cached_patch - ${paths[@]+"${paths[@]}"}
  commit_staged "replay(task409): $stage semantic projection" - ${paths[@]+"${paths[@]}"}
}
...
import subprocess
import sys
m, repo, head = json.load(open(sys.argv[1])), sys.argv[2], sys.argv[3]
def run(*args):
    return subprocess.check_output(['/usr/bin/git','-C',repo,'-c','core.hooksPath=/dev/null',*args], text=True).strip()
...
git = '/usr/bin/git'
def show(spec):
    return subprocess.check_output([git, '-C', str(repo), '-c', 'core.hooksPath=/dev/null', 'show', spec])
...
readme = subprocess.check_output(['/usr/bin/git','-C',repo,'-c','core.hooksPath=/dev/null','show',f'{head}:scripts/README.md'])
...
assert_forbidden_ancestry() {
  ...
  done < <("$PYTHON" - "$MATRIX" <<'PY'
import json
import sys
m = json.load(open(sys.argv[1]))
for value in m['forbidden_ancestry']['commits'] + m['forbidden_ancestry']['raw_semantic_source_commits']:
    print(value)
PY
)
  ...
  local bad rc
  for bad in ${bad_objects[@]+"${bad_objects[@]}"}; do
    set +e
...
cleanup_success_only() {
  ...
  for path in "$@"; do
    allowed=0
    for item in ${CLEANUP_ALLOWLIST[@]+"${CLEANUP_ALLOWLIST[@]}"}; do
      [ "$path" = "$item" ] && allowed=1
    done
...
cleanup_clean_primary_success_only() {
  ...
  allowed=0
  for item in ${CLEAN_PRIMARY_CLEANUP_ALLOWLIST[@]+"${CLEAN_PRIMARY_CLEANUP_ALLOWLIST[@]}"}; do
...
def run(*args, check=True, input_bytes=None):
    p = subprocess.run([git, '-C', repo, '-c', 'core.hooksPath=/dev/null', *args], input=input_bytes, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
    if check and p.returncode:
...
def patch_id(left, right, paths):
    ...
    p = subprocess.run([git, 'patch-id', '--stable'], input=raw, stdout=subprocess.PIPE, check=True)
...
if '--ignored=all' not in execution_driver['shell']:
...
| JSON companion | `/private/tmp/hermternal-task409-final-execution-matrix.json` |
| Driver schema | `hermternal.task409.execution-driver.v1` |
| Driver shell SHA-256 | `1168e369d9b1f2116ae871e890ff6ab3b0489ac0c6c0f0925279083c7e2240dd` |
...
| Swift source SHA | `221620c04bb051f2597c52bdeb16ccc55c5b2e9c` |
| Shell body bytes | `106051` |
| Shell body lines | `2221` |
