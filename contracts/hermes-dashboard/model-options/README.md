...
- **Source identity:** the validator binds the complete `fixture_surface` structure, including every source path, line range, builder field, provider-row note, and retained blob ID. It also binds the repository slug and URL, REST and registry provenance records, and four unique pinned source links to exact expected values. Source links contain no query strings or fragments. `validate_fixture` revalidates any audit argument before trusting it.
