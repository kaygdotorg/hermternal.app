...
- **Source identity:** the validator binds the repository slug and URL, handler and shared-builder Git blob IDs, REST and registry provenance records, and four unique pinned source links to exact expected values. Source links contain no query strings or fragments.
- **Redaction:** recursive fixture validation uses an explicit sensitive-key/value policy. It rejects credential material such as PEM private keys, AWS `AKIA` keys, GitHub PATs, and bearer/basic credentials while allowing legitimate metadata such as `authenticated` and `auth_type`; ordinary synthetic provider and model identifiers remain valid.
