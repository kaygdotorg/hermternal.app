...
| Attach keep-alive | A non-empty opaque handle selects registry mode. The timeline proves socket A detach, registry retention, socket B accept, registry reuse, and reattach with the same handle, session, and process identity. Disconnect and attach-mode Close detach the socket while retaining the process for 1800 seconds. |
...
- `validate.py` — standard-library validator. It rejects unknown keys, wrong types, duplicate fixture IDs, non-synthetic references, secret-shaped text, duplicate JSON keys, JSON `NaN`/`Infinity`, deep JSON trees, source-audit drift, and baseline content drift. Its mutation inventory is executable and must fail closed.
