...
class PtyLocalAdapterValidationTests(unittest.TestCase):
    ...
    def test_canonical_fixture_and_mutation_inventory(self) -> None:
        ...
        mutation_ids = tuple(name for name, _ in mutations)
        self.assertEqual(mutation_ids, validate.EXPECTED_MUTATION_IDS)
        self.assertEqual(len(mutation_ids), len(set(mutation_ids)))
        self.assertEqual(validate.validate_mutations(self.document), len(validate.EXPECTED_MUTATION_IDS))
    ...
    def test_byte_preservation_keeps_split_incomplete_and_invalid_frames(self) -> None:
        ...
        self.assertEqual(
            tuple((frame["frame_ref"], frame["frame_hex"]) for frame in frames),
            validate.BYTE_FRAME_ANCHORS,
        )
        self.assertEqual([frame["frame_hex"] for frame in frames], ["f0", "9f9880", "e282", "c328"])
        self.assertEqual(case["expected"]["joined_hex"], validate.BYTE_JOINED_HEX)
    ...
    def test_resize_clamps_integer_boundaries_and_rejects_malformed_types(self) -> None:
        ...
        self.assertEqual(
            tuple(
                (candidate["wire_shape"], candidate["invalid_field"], candidate["cols"], candidate["rows"], candidate["frame_text"])
                for candidate in rejection["input"]["candidates"]
            ),
            validate.RESIZE_REJECTION_ANCHORS,
        )
    ...
    def test_attach_reattach_reuses_all_identity_and_one_spawn(self) -> None:
        ...
        self.assertNotEqual(case["input"]["first_socket_ref"], case["input"]["second_socket_ref"])
        self.assertEqual(
            len({case["input"]["handle_ref"], case["input"]["session_ref"], case["input"]["process_ref"]}),
            3,
        )
        self.assertEqual(case["input"]["handle_ref"], validate.ATTACH_IDENTITY_ANCHORS[0])
        self.assertEqual(case["input"]["second_socket_ref"], validate.ATTACH_IDENTITY_ANCHORS[4])
    ...
    def test_close_codes_cover_replacement_dead_process_and_legacy_disconnect(self) -> None:
        ...
        close_case = self._case("close-codes")
        self.assertEqual(
            [event["name"] for event in close_case["input"]["legacy_events"]],
            list(validate.LEGACY_TIMELINE),
        )
        self.assertEqual(close_case["input"]["legacy_events"][3]["result"], "prohibited")
        self.assertEqual(
            [event["close_code"] for event in close_case["input"]["error_events"]],
            [code for _, _, code in validate.ERROR_CLOSE_ANCHORS],
        )
        validate.validate_cases_document(self.document)
    ...
    def test_normal_and_optimized_cli_reject_boundary_mutations(self) -> None:
        mutation_ids = {
            "byte-frame-anchor",
            "resize-prefix",
            "resize-frame-text",
        ...
        mutations = dict(validate.mutation_inventory(self.document))
        with tempfile.TemporaryDirectory() as directory:
            for mutation_id in mutation_ids:
                path = Path(directory) / f"{mutation_id}.json"
                path.write_text(json.dumps(mutations[mutation_id]), encoding="utf-8")
                for optimized in (False, True):
                    result = self._run_cli(optimized, "--cases", str(path), "--skip-baseline")
                    with self.subTest(mutation_id=mutation_id, optimized=optimized):
    ...
    def test_checked_in_baseline_is_strict_and_threshold_is_null(self) -> None:
        ...
        self.assertEqual(
            (baseline["environment"]["platform"], baseline["environment"]["python"]),
            validate.BASELINE_CANONICAL_ENVIRONMENT,
        )
        self.assertEqual(
            tuple(run["mode"] for run in baseline["runs"]),
            validate.BASELINE_MODE_ORDER,
        )
    ...
    def test_baseline_mutations_fail_closed(self) -> None:
        ...
        mutations.append(candidate)
        candidate = copy.deepcopy(baseline)
        candidate["runs"] = list(reversed(candidate["runs"]))
        mutations.append(candidate)
        candidate = copy.deepcopy(baseline)
        candidate["environment"]["platform"] = "alternate-platform"
        mutations.append(candidate)
        candidate = copy.deepcopy(baseline)
        candidate["environment"]["python"] = "3.13.0"
        mutations.append(candidate)
        candidate = copy.deepcopy(baseline)
