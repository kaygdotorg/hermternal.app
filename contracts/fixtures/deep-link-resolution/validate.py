...
CASE_IDS=(
 ...
 "latest-descendant-malformed-order-fails-closed","latest-descendant-decreasing-order-fails-closed",
 "latest-descendant-cycle-fails-closed","latest-descendant-missing-parent-fails-closed",
 "latest-descendant-wrong-root-fails-closed","latest-descendant-duplicate-node-fails-closed",
 "unknown-session-safe-not-found","unauthorized-session-safe-not-found",
...
# This anchor is outside the mutable local proof set. The review root binds the
# validator exactly, so local artifacts, source, root, and local digest copies
# cannot authorize a coordinated replacement.
REVIEW_ANCHOR_PATH=REPO_ROOT/"contracts/fixtures/review-anchors/deep-link-resolution.sha256"
...
def read_artifact_once(path:Path,limit:int=MAX_FILE_BYTES)->Artifact:
 """Reject special paths before a nonblocking, stable regular-file read."""
 try:
  path_before=os.stat(path,follow_symlinks=False)
  require(stat.S_ISREG(path_before.st_mode) and not stat.S_ISLNK(path_before.st_mode) and path_before.st_size<=limit)
...
class Resolver:
 ...
 opened_session_id:str|None=None; root_id:str|None=None; parent_id:str|None=None; focused_message_id:str|None=None; focus:str="none"; decision:str="pending"
 ...
 def apply(self,event:Mapping[str,Any],lineage:Mapping[str,tuple[str,str|None]])->None:
  require(type(event)is dict and list(event) in (["type","at_seconds"],["type","at_seconds","link"])); kind=event["type"]; now=event["at_seconds"]; require(type(kind)is str and type(now)is int and 0<=now<=MAX_INTEGER)
  allowed={"receive","authenticated","lookup_present","lookup_missing","lookup_denied","message_present","message_missing","complete","reload","interrupt","recover","expire","logout","cancel"}; require(kind in allowed); require(list(event)==(["type","at_seconds","link"] if kind=="reload" else ["type","at_seconds"]))
  if kind not in {"receive","reload","expire"} and self.expire_due(now): return
  if kind=="receive": require(list(event)==["type","at_seconds"] and self.state=="idle"); self.receive(now)
  elif kind=="authenticated": require(list(event)==["type","at_seconds"] and self.state=="auth_pending" and self.deadline_seconds is not None); self.authentication_checks+=1; self.lookup_attempts+=1; self.effects.extend(("authentication_confirmed","authenticated_lookup_started")); self.transition("lookup_pending")
  elif kind=="lookup_present":
   require(list(event)==["type","at_seconds"] and self.state=="lookup_pending" and self.pending_session_id is not None)
   try:
  ...
  elif kind in {"lookup_missing","lookup_denied"}: require(list(event)==["type","at_seconds"] and self.state=="lookup_pending"); self.decision="session_not_found"; self.effects.extend(("authorization_safe_session_not_found","no_session_created")); self.transition("failed"); self.cleanup()
   ...
   require(list(event)==["type","at_seconds"] and self.deadline_seconds is not None)
   if not self.expire_due(now): self.effects.append("deadline_not_reached")
  elif kind=="logout": require(list(event)==["type","at_seconds"] and self.pending_session_id is not None); self.authenticated_context=False; self.decision="signed_out"; self.transition("signed_out"); self.cleanup()
  elif kind=="cancel": require(list(event)==["type","at_seconds"] and self.pending_session_id is not None); self.decision="cancelled"; self.transition("cancelled"); self.cleanup()
  else: raise ContractError("contract rejected")
 def result(self)->dict[str,Any]:
  return {"decision":self.decision,"final_state":self.state,"state_trace":self.state_trace,"effects":self.effects,"link_input":self.link_input,"opened_session_id":self.opened_session_id,"root_id":self.root_id,"parent_id":self.parent_id,"focused_message_id":self.focused_message_id,"focus":self.focus,"pending_link":self.pending_link,"pending_session_id":self.pending_session_id,"pending_message_id":self.pending_message_id,"deadline_seconds":self.deadline_seconds,"lookup_attempts":self.lookup_attempts,"parse_attempts":self.parse_attempts,"authentication_checks":self.authentication_checks,"session_creations":0,"shares":0,"transcript_mirror":False,"network":False}
...
def validate_expected(value:Any)->None:
 ...
 for key in ("link_input","opened_session_id","root_id","parent_id","focused_message_id","pending_link","pending_session_id","pending_message_id"): require(value[key] is None or type(value[key])is str)
 ...
 if value["final_state"] in {"opened","failed","expired","signed_out","cancelled"}: require(all(value[key] is None for key in ("link_input","pending_link","pending_session_id","pending_message_id","deadline_seconds")))
 if value["decision"]=="message_focused": require(value["focused_message_id"]==MESSAGE_ID)
...
def validate_mutations(document:Mapping[str,Any],store:ArtifactStore|None=None)->int:
 mutations=(
  lambda v:v.update({"extra":True}),lambda v:v.update({"pending_ttl_seconds":True}),lambda v:v["dependencies"]["deep-link-grammar/cases.json"].update({"sha256":"0"*64}),lambda v:v["cases"].reverse(),lambda v:v["cases"][0].update({"authenticated":1}),lambda v:v["cases"][0]["events"][0].update({"at_seconds":True}),lambda v:v["cases"][0]["events"][1].update({"at_seconds":0}),lambda v:v["cases"][0]["expected"].update({"decision":"pending"}),lambda v:v["cases"][0]["expected"].update({"session_creations":1}),lambda v:v["cases"][0]["expected"].update({"pending_session_id":ROOT_ID}),lambda v:v["cases"][0]["expected"].update({"deadline_seconds":1300}),lambda v:v["cases"][18]["expected"].update({"parse_attempts":1}),lambda v:v["cases"][18]["expected"].update({"authentication_checks":1}),lambda v:v["cases"][2]["lineage_ordering"][1].update({"sequence":0}),lambda v:v["cases"][3]["expected"].update({"decision":"session_opened"}),lambda v:v["cases"][4]["expected"].update({"decision":"session_opened"}),lambda v:v["cases"][14]["events"][1].update({"at_seconds":1299}),lambda v:v["cases"][11]["expected"].update({"decision":"denied"}),lambda v:v["redaction"]["fixed_error"]["error"].update({"message":ROOT_ID}),lambda v:v["cases"][0]["expected"].update({"network":True}),
 )
