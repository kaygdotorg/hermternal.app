...
41,693,163-byte macOS fresh-clone pack, so the 40 MiB per-pack cap leaves
249,877 bytes of measured headroom. A seeded clone retains two packs plus
metadata; a measured seeded snapshot totals 79,918,215 bytes, and the independent 80 MiB aggregate cap bounds that composition without
