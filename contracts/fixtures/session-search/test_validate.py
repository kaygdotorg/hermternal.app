...
class SessionSearchTests(unittest.TestCase):
 ...
 def test_checked_in_contract_and_baseline_validate(self):
  count,size=validate.validate_all(self.document,self.baseline,self.cases_raw,self.baseline_raw);self.assertEqual(count,45);self.assertGreater(size,0);self.assertEqual(self.baseline["repetitions"],30);self.assertIsNone(self.baseline["threshold"])
 def test_normal_and_optimized_cli_have_exact_parity(self):
  normal=self.run_cli();optimized=self.run_cli(True);self.assertEqual((normal.returncode,normal.stdout,normal.stderr),(0,optimized.stdout,""));self.assertEqual(optimized.stderr,"");self.assertIn("cases=45",normal.stdout)
 ...
 def test_order_tie_break_and_real_input_permutations_are_stable(self):
  ...
  request=self.cases["stable-repeatability"]["request"];catalog=self.cases["stable-repeatability"]["catalog"];expected=json.dumps(validate._execute(request,catalog),sort_keys=True,separators=(",",":"))
  for permutation in (list(reversed(catalog)),catalog[1:]+catalog[:1],[catalog[i] for i in(2,0,3,1)]):self.assertEqual(json.dumps(validate._execute(copy.deepcopy(request),permutation),sort_keys=True,separators=(",",":")),expected)
 def test_cursor_is_nonsemantic_integrity_bound_and_catalog_bound(self):
  ...
  for case_id in("cursor-query-mismatch","cursor-mode-mismatch","cursor-catalog-drift","cursor-forgery","malformed-cursor","lone-surrogate-cursor"):self.assertEqual(self.cases[case_id]["expected"]["error"]["code"],"invalid_cursor")
 ...
 def test_retry_requires_bound_prior_and_reconciliation_evidence(self):
  ...
  self.assertEqual(self.cases["interrupted-forged-proof"]["expected"]["error"]["code"],"retry_blocked");self.assertEqual(self.cases["unknown-response-prior-only-blocked"]["expected"]["error"]["code"],"retry_blocked");self.assertEqual(self.cases["reconciliation-catalog-drift"]["expected"]["error"]["code"],"retry_blocked")
 ...
 def test_unknown_argparse_and_surrogates_are_fixed_redacted_in_both_modes(self):
  ...
  for case_id in("lone-surrogate-query","lone-surrogate-cursor"):
   ...
   for optimized in(False,True):
    code="import json,sys;sys.path.insert(0,"+repr(str(ROOT))+');import validate;print(json.dumps(validate._execute('+repr(request)+","+repr(self.cases[case_id]["catalog"])+"),ensure_ascii=True,sort_keys=True))"
 ...
 def test_dependencies_source_pin_and_redaction_are_canonical(self):
  self.assertEqual(self.document["hermes_source_sha"],validate.SOURCE_SHA);self.assertEqual(self.document["dependencies"],validate.DEPENDENCIES);self.assertEqual(self.document["source_observations"],validate.SOURCE_OBSERVATIONS);validate._validate_dependencies()
