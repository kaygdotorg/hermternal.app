...
| **M0 — policy model** | A deterministic route, header, fixture, or renderer model evaluated without the target runtime. | Accepted and denied vectors, normalized route policy, bounded header decisions, and redacted evidence schema. | Runtime parser compatibility, deployed middleware behavior, upstream logs, firewall behavior, or live service responses. |
...
- `apps/web/playwright.config.ts` remains the ordinary local lane: it uses the standard list reporter and retains failure traces. It is not a credential-bearing proof boundary. Selecting it for a credential-bearing run is a release blocker unless a separate redaction contract is reviewed.
...
- The Caddy and Traefik proof fixtures are separate model or synthetic-local boundaries, not deployment log proof. The Caddy renderer records redaction policy markers but no concrete access-log redaction directive was established in this audit. The Traefik renderer configures bounded JSON/status logging and a local ForwardAuth policy model, but does not establish field-level redaction or runtime compatibility. Their checked-in evidence correctly records blocked or unproven browser, cookie, and runtime claims. DEP-12 therefore remains open for real edge, access, error, upstream, malformed-message, and provider-log collection with non-sensitive canaries.
...
Run the proxy-policy and disposable-fixture checks. These use local renderers,
synthetic evidence, and bounded loopback test doubles; they do not start a
proxy, Hermes, provider, browser, or network service:
...
python3 -m py_compile scripts/caddy_proof.py scripts/test_caddy_proof.py
...
| A synthetic fixture or policy model is mistaken for a live provider or deployment result. | Label it S1 or M0, keep it provider-free, and require L0 plus R-02/R-03/R-04 evidence for live claims. | `apps/web/README.md`, `apps/web/tests/live/README.md`, proxy proof READMEs, and this taxonomy. |
...
- a credential-bearing run selects the ordinary Playwright configuration with its standard reporter or retained failure traces instead of the reviewed live boundary;
- a runner can bypass the IPC guard, inherit debug output, recreate post-teardown metadata, or write outside the owned root;
