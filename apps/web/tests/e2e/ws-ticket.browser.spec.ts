...
const sourcePath = fileURLToPath(new URL('../../src/lib/chat/ws-ticket.ts', import.meta.url));
const bundlePath = join(tmpdir(), `hermternal-ws-ticket-${process.pid}-${randomUUID()}.mjs`);
execFileSync('bun', [
  'build',
  sourcePath,
  '--target=browser',
...
test('real browser acquisition keeps the ticket only in the ephemeral upgrade URL', async ({ page, baseURL }) => {
    ...
    async ({ moduleUrl, expectedTicket }) => {
      ...
      try {
        const client = module.createWsTicketClient({
          ...
          connect: async (upgradeUrl: URL) => ({
            sameOrigin:
              ...
              ((upgradeUrl.protocol === 'ws:' && location.protocol === 'http:') ||
                (upgradeUrl.protocol === 'wss:' && location.protocol === 'https:')),
            ...
            expectedTicket: upgradeUrl.searchParams.get('ticket') === expectedTicket
          })
        });
        ...
        return {
          ...
          logsRedacted: !consoleMessages.some((message) => message.includes(expectedTicket))
        };
      } finally {
...
test('real browser cancellation does not create an upgrade or an automatic retry', async ({ page }) => {
  let requestCount = 0;
  await page.route('**/api/auth/ws-ticket', async (route) => {
    requestCount += 1;
  ...
  expect(result).toEqual({ code: 'cancelled', name: 'AbortError', connectCount: 0 });
});

test('real browser retry is explicit and redacts an authentication response', async ({ page }) => {
    ...
    async ({ moduleUrl, marker }) => {
      ...
      return {
        ...
        firstMessageRedacted:
          firstError instanceof Error ? !firstError.message.includes(marker) : false,
        countBeforeRetry,
        secondResult,
        connectCount
      };
