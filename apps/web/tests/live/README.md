...
The approved Paper logout states now back the root browser composition. The non-live `tests/e2e/live-root.spec.ts` test proves the visible account-menu callback with mocked same-origin responses and no Hermes connection. The opt-in official live proof still verifies the reviewed same-origin logout boundary as raw `302 Location: /login`, then verifies the follow-up `/api/auth/me` `401`; it remains credential-gated and is not required for the mocked UI test lane.
