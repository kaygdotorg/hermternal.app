...
`shared-session-live-proof.spec.ts` is a separate credential-gated lane for the Chat → Terminal → Chat journey. It uses one Chat socket and one PTY upgrade, commits a fresh draft only through the `stored_session_id` returned by `session.create` (the ephemeral create ID is ignored for continuity), verifies the first marker in authoritative REST history, resumes PTY with that committed identity, returns to Chat without a new session or transport, and reconciles both marker pairs through final REST history. Its companion ledger is value-free: it retains only redacted routes, methods, event names, statuses, bounded lengths/counts, close codes, and boolean identity/content matches. The proof correlates `message.delta` and `message.complete` to the active prompt when Hermes omits optional wire `session_id` or `request_id`; those raw fields are never retained. It waits for both streaming and completion evidence and rechecks the sanitized runtime preview state after each authoritative refresh. The focused proof demonstrates shared Hermes session continuity across the workspace modes; it does not demonstrate exact PTY-process reattachment. The mocked workspace regression covers the same owner, explicit legacy reattach prohibition, and no-byte-retention contract without live credentials or inference. Both credential-bearing files run with `workers: 1`; the first Chat-only proof remains independent and is not given a Terminal prerequisite.
