...
The host is test-only. It is not a deployment server and does not add authentication, retries, transcript storage, or response logging. The live Playwright configuration writes to a unique OS-temporary output directory, sets `preserveOutput: 'never'`, disables traces, videos, and screenshots, uses the safe status-only reporter, and removes the output root during both per-test and global teardown. The live fixture scrubs input, textarea, select, and every editable DOM mode (`true`, empty, and `plaintext-only`) before page close. It redacts known synthetic credentials and serialized form values from bounded structured Playwright diagnostics, including non-enumerable native Error message/stack/cause fields, the string `TestInfoError.errorContext`, matcher results, logs, and ARIA snapshots; cycles, throwing accessors, rejected or silent writes, and malformed or over-budget values fail closed. Serialized contenteditable markup requires balanced tags and terminated comments. Attachment references and temporary output are removed in `finally` even when redaction fails. A scrub evaluator failure is suppressed only when `page.isClosed()` returns `true`; generic error text such as `page crashed` is never treated as proof of termination. These layers are deliberate: a failed live assertion must not leave a password in a trace, screenshot, report, error context, DOM dump, or the repository's retained `test-results` directory. The spec records only HTTP method/path pairs and JSON-RPC method or event names. It never records the WebSocket ticket, credential value, prompt response, cookie, or raw frame payload.
