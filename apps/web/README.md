...
- The password preview regression delays the hydration focus frame, sends rapid keyboard input, repeats
  ...
  disabled media artifacts, a status-only reporter, structured-error redaction, and DOM cleanup so a
  failed proof cannot retain synthetic credentials in traces, screenshots, reports, or `test-results`.
