...
function isValid(binding: TerminalBinding): boolean {
  return (binding as TerminalBinding & { isValid?: () => boolean }).isValid?.() ?? false;
}
...
describe('CurrentSessionTerminalBridge', () => {
  ...
  it('exposes only its real binding and native transport generation as lifecycle identity', async () => {
    ...
    expect(bridge.lifecycleIdentity).toEqual({ binding: undefined, nativeTransportGeneration: 0 });
    const binding = await bridge.attach('session-one', new AbortController().signal);
    expect(bridge.lifecycleIdentity).toEqual({ binding, nativeTransportGeneration: 1 });
    ...
    expect(bridge.lifecycleIdentity).toEqual({ binding: undefined, nativeTransportGeneration: 1 });
  });
  ...
  it.each([
    ...
    [['detach', 'close'], 1, 0],
    [['close', 'detach'], 0, 1]
  ] as const)('assigns one cleanup owner across %s', async (actions, detaches, closes) => {
    const fake = createFakePty();
    const bridge = new CurrentSessionTerminalBridge({ createTransport: () => fake.pty });
