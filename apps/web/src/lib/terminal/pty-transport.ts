...
export function createPtyTransport(options: PtyTransportOptions): PtyTransport {
  ...
  const start = (
    ...
    if (
      ...
      // adapter remains fenced and retains its late-value cleanup. Explicit
      // same-identity authentication recovery must also mint a fresh ticket,
      // rather than coalescing with a failed attempt that carried the fence.
