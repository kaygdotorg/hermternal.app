...
The live Hermes gateway does not provide one total order for REST, JSON-RPC replies, events, and socket lifecycle callbacks. The earlier synthetic fixture assumed a request acknowledgement preceded prompt events; official Hermes can start prompt work and emit completion-related events before returning that acknowledgement. A generic failed or uncertain callback can also arrive after a successful completion and its REST read. The workspace therefore commits a bounded history barrier only after the exact generation, selected session, and chat transport have accepted a REST replacement. A late generic callback cannot regress that committed view; a failure while the read is still pending still wins and rejects the late response. A persisted restore follows the same rule: REST history is displayed only as a stable restore after gateway connection and `session.resume` succeed, so a pre-ticket or pre-handshake failure remains retryable. Authentication-required and incompatible-origin classifications always bypass this generic barrier.
